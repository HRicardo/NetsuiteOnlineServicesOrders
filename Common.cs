                        ...
                        if (string.IsNullOrEmpty(strId))
                        {
                            ...
                            if (!string.IsNullOrEmpty(strCustomer) && !string.IsNullOrEmpty(strProduct) && !string.IsNullOrEmpty(strPoNumber))
                            {
                                ...
                                if (numIndex + 1 >= dsTable.Rows.Count || !strNextRef.Equals(strPoNumber))
                                {
                                    // Insert order
                                    string strError = "";
                                    strId = Util.AddSalesOrder(arrReseller.IndexOf(strCustomer) + 1, objSubCustomer, strPoNumber, strPoNumber, objOrderDate, DateTime.Now, objOrderDate, arrItems, strEmployeeId, strInvoiceDate, out strError);
                                    if (!string.IsNullOrEmpty(strId))
                                    {
